...
if (TRUE) {
  dat <- readRDS("../RAATD_01/RAATD/meanPredictions/griddedScoresEnvarsTransformedColonyWeighted.RDS")
} else {
  dat <- readRDS("../RAATD_01/RAATD/meanPredictions/griddedScoresEnvarsTransformed.RDS")
}
...
# In the revised analysis, hot and coldspots are ignored
...
datS$group2 <- paste0(datS$group)
datS$group2 <- paste(formatC(datS$group, width=2, flag="0"))

# Create a combined palette that groups cold clusters
#pal <- c(sequential_hcl(k.cold, palette = "Light Grays"), glasbey(k.hot))

# Replace this with a full-color palette for revisions, ignoring hot and coldspots,
pal <- glasbey(this_k)
...
plot(hcl, hang = -0.00000000001, axes = F)
...
vec.sp.df$label <- rank(-env_fit$vectors$r)
# vec.sp.df$label <- paste(formatC(vec.sp.df$label, width=2, flag="0"))

# Mutliplier for scaling the vector output
mult <- vegan:::ordiArrowMul(env_fit)

dat_sub <- dat_sub %>%
  mutate(supercluster = case_when(group2 == "13" ~ "Antarctic",
                                 group2 == "15" ~ "Antarctic",
                                 group2 == "16" ~ "Antarctic",
                                 group2 == "14" ~ "Antarctic",
                                 group2 == "17" ~ "Antarctic",
                                 group2 == "09" ~ "Scotia Arc",
                                 group2 == "10" ~ "Scotia Arc",
                                 group2 == "03" ~ "Distant Subantarctic",
                                 group2 == "08" ~ "Distant Subantarctic",
                                 group2 == "04" ~ "Distant Subantarctic",
                                 group2 == "01" ~ "Distant Subantarctic",
                                 group2 == "02" ~ "Distant Subantarctic",
...
# datSub <- unite(data = datSub, col = "super_group_cluster", c("super_group", "group2"), sep = "_", remove = FALSE)
                                  

# Creat convex hulls
hull_antarctic <- dat_sub %>%
  filter(., supercluster == "Antarctic") %>% 
  ungroup(.) %>% 
  slice(chull(MDS1, MDS2))

hull_subantarctic <- dat_sub %>%
  filter(., supercluster == "Subantarctic") %>% 
  ungroup(.) %>% 
  slice(chull(MDS1, MDS2))

hull_scotia <- dat_sub %>%
  filter(., supercluster == "Scotia Arc") %>% 
  ungroup(.) %>% 
  slice(chull(MDS1, MDS2))

hull_distant <- dat_sub %>%
  filter(., supercluster == "Distant Subantarctic") %>% 
  ungroup(.) %>% 
  slice(chull(MDS1, MDS2))

pdf("./plots/mds.pdf", width = 6, height = 6, useDingbats = FALSE)
ggplot(data = dat_sub, aes(x = MDS1, y = MDS2, colour = as.factor(group2))) +
  geom_polygon(data = hull_antarctic, aes(x = MDS1, y = MDS2, group = supercluster), colour = "grey70", fill = NA, inherit.aes = F) + 
  geom_polygon(data = hull_subantarctic, aes(x = MDS1, y = MDS2, group = supercluster), colour = "grey70", fill = NA, inherit.aes = F) + 
  geom_polygon(data = hull_scotia, aes(x = MDS1, y = MDS2, group = supercluster), colour = "grey70", fill = NA, inherit.aes = F) + 
  geom_polygon(data = hull_distant, aes(x = MDS1, y = MDS2, group = supercluster), colour = "grey70", fill = NA, inherit.aes = F) + 
  geom_point(alpha = 0.7, size = 0.7) +
  scale_color_manual(values = pal, name = "Cluster") +
   # geom_segment(data = vec.sp.df, aes(x = 0, xend = NMDS1*mult, y = 0, yend = NMDS2*mult),
  #              arrow = arrow(length = unit(0.3, "cm")), colour = "black", inherit.aes = FALSE) + 
  geom_segment(data = vec.sp.df, aes(x = 0, xend = NMDS1*mult, y = 0, yend = NMDS2*mult), colour = "black", inherit.aes = FALSE) +
    # geom_label(data = vec.sp.df, aes(x = NMDS1*mult, y = NMDS2*mult, label = env), size = 2.5, inherit.aes = FALSE,
    #          alpha = 0.5, label.r = unit(0, "lines")) +
  geom_label(data = vec.sp.df, aes(x = NMDS1*mult, y = NMDS2*mult, label = label), size = 2.5, inherit.aes = FALSE, label.r = unit(0, "lines")) +
  coord_fixed(1) +
  labs(x = "Dimension 1", y = "Dimension 2") +
  annotate(geom = "text", x = - 0.30, y = -0.27, label = paste0("Stress: ", round(mds$stress, 2)), size = 2.5) +
  guides(color = guide_legend(override.aes = list(size=2))) + # bigger legend symbols
  theme_bw() +
  theme(
...
# Small inset of polygons
pdf("./plots/mds_inset.pdf", width = 6, height = 6, useDingbats = FALSE)
ggplot(data = dat_sub, aes(x = MDS1, y = MDS2, colour = as.factor(group2))) +
  geom_polygon(data = hull_antarctic, aes(x = MDS1, y = MDS2, group = supercluster), color = "grey30", fill = "grey70", alpha = 0.5, inherit.aes = F) + 
  geom_polygon(data = hull_subantarctic, aes(x = MDS1, y = MDS2, group = supercluster), color = "grey30", fill = "grey70", alpha = 0.5,  inherit.aes = F) + 
  geom_polygon(data = hull_scotia, aes(x = MDS1, y = MDS2, group = supercluster), color = "grey30", fill = "grey70", alpha = 0.5, inherit.aes = F) + 
  geom_polygon(data = hull_distant, aes(x = MDS1, y = MDS2, group = supercluster), color = "grey30", fill = "grey70", alpha = 0.5, inherit.aes = F) + 
  geom_segment(data = vec.sp.df, aes(x = 0, xend = NMDS1*mult, y = 0, yend = NMDS2*mult), colour = "black", inherit.aes = FALSE) +
  coord_fixed(1) +
  theme_bw() +
  theme(
...
env_fit_results$order <- 1:nrow(env_fit_results)
write.csv(env_fit_results, "./dat_out/mds_envfit_results.csv", row.names = F)
